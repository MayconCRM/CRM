...
const KPICard = ({ title, value, icon: Icon, color = "#f59e0b", trend, subtitle }) => (
  <Card className="relative overflow-hidden">
    ...
    <div className="flex items-start justify-between mb-2">
      ...
      {trend && <span className={`text-xs font-medium ${trend > 0 ? "text-emerald-400" : "text-red-400"}`}>{trend > 0 ? "+" : ""}{trend}%</span>}
    </div>
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: "linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)" }}>
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="bg-slate-900/80 backdrop-blur border border-slate-700/50 rounded-2xl p-6 shadow-2xl">
          ...
          <p className="text-xs text-slate-500 mb-4">{dbUsers ? "✓ Conectado ao Supabase" : "Conectando..."}</p>
  ...
  return (
    <aside className={`fixed left-0 top-0 h-screen bg-slate-950 border-r border-slate-800 z-40 flex flex-col transition-all duration-300 ${collapsed ? "w-16" : "w-56"}`}>
      ...
      <nav className="flex-1 py-2 px-2 overflow-y-auto">
        {menuItems.map(item => (
          <button key={item.id} onClick={() => setCurrentPage(item.id)}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all mb-0.5 ${currentPage === item.id ? "bg-amber-600/20 text-amber-400 border border-amber-500/20" : "text-slate-400 hover:text-white hover:bg-slate-800/50"}`}>
            <item.icon size={18} />
      ...
      <div className="p-3 border-t border-slate-800">
        <button onClick={() => setCollapsed(!collapsed)} className="w-full flex items-center justify-center p-2 text-slate-500 hover:text-white hover:bg-slate-800 rounded-lg transition-colors">
...
const DashboardPage = ({ data }) => {
  ...
  const mrr = contAtivos.reduce((s, c) => s + (parseFloat(c.valor_mensal) || 0), 0);
  ...
  const vendedorData = SEED_USERS.filter(u => u.perfil === "comercial").map(u => ({
    ...
    receita: oportunidades.filter(o => o.responsavel_id === u.id && o.etapa === "ganho").reduce((s, o) => s + (parseFloat(o.valor_mensal) || 0), 0),
  }));
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={funnelData} layout="vertical">
              ...
              <Bar dataKey="value" radius={[0, 4, 4, 0]}>{funnelData.map((entry, i) => <Cell key={i} fill={entry.fill} />)}</Bar>
            </BarChart>
          </ResponsiveContainer>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={prodData.length > 0 ? prodData : [{ name: "Sem dados", value: 1, color: "#334155" }]} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={5} dataKey="value">
                {(prodData.length > 0 ? prodData : [{ color: "#334155" }]).map((entry, i) => <Cell key={i} fill={entry.color} />)}
              </Pie>
              <Tooltip contentStyle={{ background: "#1e293b", border: "1px solid #334155", borderRadius: "8px", fontSize: "12px", color: "#fff" }} />
...
const PipelinePage = ({ data, setData }) => {
  ...
  const saveOp = (form) => {
    if (editOp) {
      setData(prev => ({ ...prev, oportunidades: prev.oportunidades.map(o => o.id === editOp.id ? { ...o, ...form } : o) }));
    } else {
      setData(prev => ({ ...prev, oportunidades: [...prev.oportunidades, { ...form, id: uid(), etapa: "prospecção" }] }));
    }
    setShowForm(false);
    setEditOp(null);
  };
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3 justify-between">
        <div className="flex gap-2 flex-wrap">
          <Select value={filterResp} onChange={setFilterResp} options={SEED_USERS.filter(u => u.perfil === "comercial").map(u => ({ value: u.id, label: u.nome }))} placeholder="Todos responsáveis" />
          <Select value={filterProd} onChange={setFilterProd} options={TIPOS_PRODUTO.map(t => ({ value: t.value, label: t.label }))} placeholder="Todos produtos" />
        </div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-4" style={{ minHeight: "60vh" }}>
        {ETAPAS.map(etapa => {
          ...
          const total = cards.reduce((s, c) => s + (parseFloat(c.valor_mensal) || 0), 0);
          return (
                  ...
                  return (
                    <div key={op.id} draggable onDragStart={() => setDragId(op.id)} onDragEnd={() => setDragId(null)}
                      ...
                      <div className="flex items-start justify-between mb-1.5">
                        ...
                        <Badge color={getProductColor(op.produto)}>{getProductLabel(op.produto)}</Badge>
                      </div>
                      <p className="text-[10px] text-slate-500 mb-2">{op.solicitante}</p>
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-amber-400">R$ {(parseFloat(op.valor_mensal) || 0).toLocaleString("pt-BR")}/mês</span>
                        {resp && <div className="w-5 h-5 bg-slate-700 rounded-full flex items-center justify-center" title={resp.nome}><span className="text-[9px] text-slate-300">{resp.nome[0]}</span></div>}
                      </div>
                    </div>
                  );
                })}
      ...
      <Modal isOpen={showForm} onClose={() => { setShowForm(false); setEditOp(null); }} title={editOp ? "Editar Oportunidade" : "Nova Oportunidade"} size="lg">
...
const OpportunityDetail = ({ op, data, setData, onEdit }) => {
  const resp = SEED_USERS.find(u => u.id === op.responsavel_id);
  ...
  const addAtividade = () => {
    const a = { ...newAct, id: uid(), oportunidade_id: op.id, cliente_id: op.cliente_id };
  ...
  const vm = parseFloat(op.valor_mensal) || 0;
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div><span className="text-[10px] text-slate-500 uppercase">Valor Mensal</span><p className="text-sm text-amber-400 font-bold">R$ {vm.toLocaleString("pt-BR")}</p></div>
        ...
        <div><span className="text-[10px] text-slate-500 uppercase">MRR</span><p className="text-sm text-white">R$ {vm.toLocaleString("pt-BR")}</p></div>
        <div><span className="text-[10px] text-slate-500 uppercase">Valor Total</span><p className="text-sm text-white">R$ {(vm * (op.tempo_contrato || 0)).toLocaleString("pt-BR")}</p></div>
      ...
      <div className="flex gap-2"><Button onClick={onEdit} icon={Edit2} variant="secondary" size="sm">Editar</Button></div>
        ...
        {showActForm && (
          <div className="bg-slate-800/50 rounded-lg p-3 mb-3 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <Select label="Tipo" value={newAct.tipo} onChange={v => setNewAct(p => ({ ...p, tipo: v }))} options={[{ value: "ligacao", label: "Ligação" }, { value: "reuniao", label: "Reunião" }, { value: "email", label: "Email" }, { value: "proposta", label: "Proposta" }]} />
          ...
          {atividades.sort((a, b) => new Date(b.data_atividade) - new Date(a.data_atividade)).map(a => {
            ...
            return (
              <div key={a.id} className="flex gap-3 p-2 rounded-lg hover:bg-slate-800/30">
                <div className="flex-shrink-0 w-7 h-7 bg-slate-800 rounded-full flex items-center justify-center mt-0.5"><Ic size={12} className="text-amber-400" /></div>
...
const OpportunityForm = ({ initial, onSave, onCancel }) => {
  const [form, setForm] = useState(initial || { empresa: "", solicitante: "", email: "", endereco: "", coordenadas: "", velocidade: "", produto: "fibra", valor_mensal: "", tempo_contrato: "", observacoes: "", responsavel_id: "" });
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        ...
        <Select label="Produto" value={form.produto} onChange={v => set("produto", v)} options={TIPOS_PRODUTO.map(t => ({ value: t.value, label: t.label }))} />
        ...
        <Select label="Responsável" value={form.responsavel_id} onChange={v => set("responsavel_id", v)} options={SEED_USERS.filter(u => u.perfil === "comercial").map(u => ({ value: u.id, label: u.nome }))} />
      </div>
...
const ClientesPage = ({ data, setData }) => {
  ...
  const save = (form) => {
    if (editItem) { setData(prev => ({ ...prev, clientes: prev.clientes.map(c => c.id === editItem.id ? { ...c, ...form } : c) })); }
    else { setData(prev => ({ ...prev, clientes: [...prev.clientes, { ...form, id: uid() }] })); }
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 justify-between flex-wrap">
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar clientes..." className="pl-9 pr-3 py-2 bg-slate-800/50 border border-slate-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500/50 w-64" />
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-slate-700">
            {["Nome", "Email", "Telefone", "Cidade", "Operadora", ""].map(h => <th key={h} className="text-left py-2.5 px-3 text-xs font-medium text-slate-400 uppercase">{h}</th>)}
          </tr></thead>
          <tbody>
            {filtered.map(c => (
      ...
      <Modal isOpen={showForm} onClose={() => { setShowForm(false); setEditItem(null); }} title={editItem ? "Editar Cliente" : "Novo Cliente"} size="lg">
      ...
      <Modal isOpen={!!selected} onClose={() => setSelected(null)} title={selected?.nome || ""} size="lg">
        {selected && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[["Email", selected.email], ["Telefone", selected.telefone], ["Endereço", selected.endereco], ["Cidade", `${selected.cidade || ""} ${selected.estado ? "- " + selected.estado : ""}`], ["CEP", selected.cep], ["Operadora Atual", selected.operadora_atual]].map(([l, v]) => (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[["Técnico", "responsavel_tecnico"], ["Financeiro", "responsavel_financeiro"], ["Comercial", "responsavel_comercial"]].map(([label, prefix]) => (
                  <div key={prefix} className="bg-slate-800/30 rounded-lg p-3">
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-slate-300 mt-1"><Mail size={10} className="text-slate-500 flex-shrink-0" /><span>{selected[`${prefix}_email`] || "—"}</span></div>
                    <div className="flex items-center gap-1.5 text-xs text-slate-300 mt-0.5"><Phone size={10} className="text-slate-500 flex-shrink-0" /><span>{selected[`${prefix}_telefone`] || "—"}</span></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="border-t border-slate-700 pt-3">
              ...
              {contratos.filter(ct => ct.cliente_id === selected.id).map(ct => (
                <div key={ct.id} className="flex items-center justify-between p-2 bg-slate-800/30 rounded-lg mb-1">
                  ...
                  <span className="text-xs text-amber-400">R$ {(parseFloat(ct.valor_mensal) || 0).toLocaleString("pt-BR")}/mês</span>
                </div>
              ))}
            </div>
          </div>
...
const ContratosPage = ({ data, setData }) => {
  ...
  const getDataFim = (c) => { if (!c.data_ativacao || !c.prazo_meses) return null; const d = new Date(c.data_ativacao); d.setMonth(d.getMonth() + parseInt(c.prazo_meses)); return d; };
  const enriched = contratos.map(c => { const dataFim = getDataFim(c); const hoje = new Date(); const diasRestantes = dataFim ? Math.ceil((dataFim - hoje) / (1000*60*60*24)) : null; const status = diasRestantes !== null && diasRestantes <= 30 && diasRestantes > 0 ? "prestes_a_vencer" : c.status; return { ...c, dataFim, diasRestantes, statusCalc: status }; });
  const filtered = enriched.filter(c => { if (search && !c.empresa.toLowerCase().includes(search.toLowerCase())) return false; if (filterStatus && c.statusCalc !== filterStatus) return false; return true; });

  const save = (form) => {
    if (editItem) { setData(prev => ({ ...prev, contratos: prev.contratos.map(c => c.id === editItem.id ? { ...c, ...form } : c) })); }
    else { setData(prev => ({ ...prev, contratos: [...prev.contratos, { ...form, id: uid() }] })); }
    setShowForm(false); setEditItem(null);
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 justify-between flex-wrap">
        <div className="flex gap-2 flex-wrap items-center">
          <div className="relative"><Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar contratos..." className="pl-9 pr-3 py-2 bg-slate-800/50 border border-slate-700 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500/50 w-56" /></div>
          <Select value={filterStatus} onChange={setFilterStatus} options={[{ value: "ativo", label: "Ativo" }, { value: "prestes_a_vencer", label: "Prestes a vencer" }]} placeholder="Todos status" />
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-slate-700">
            {["Empresa", "Produto", "Valor/mês", "Prazo", "Fim", "Proteção", "Multa", "Status", ""].map(h => <th key={h} className="text-left py-2.5 px-3 text-xs font-medium text-slate-400 uppercase">{h}</th>)}
          </tr></thead>
          <tbody>
            {filtered.map(c => {
              ...
              return (
                <tr key={c.id} className="border-b border-slate-800 hover:bg-slate-800/30 cursor-pointer" onClick={() => setSelected(c)}>
                  ...
                  <td className="py-2.5 px-3 text-amber-400 font-medium">R$ {(parseFloat(c.valor_mensal) || 0).toLocaleString("pt-BR")}</td>
                  ...
                  <td className="py-2.5 px-3"><Badge color={protecao ? "#10b981" : "#ef4444"}>{protecao ? "Protegido" : "Desprotegido"}</Badge></td>
                  <td className="py-2.5 px-3 text-slate-300 font-medium">{c.multa_rescisao}</td>
                  <td className="py-2.5 px-3"><Badge color={c.statusCalc === "prestes_a_vencer" ? "#f59e0b" : "#10b981"}>{c.statusCalc === "prestes_a_vencer" ? "Vencendo" : "Ativo"}</Badge></td>
                  <td className="py-2.5 px-3" onClick={e => e.stopPropagation()}><button onClick={() => { setEditItem(c); setShowForm(true); }} className="p-1 hover:bg-slate-700 rounded"><Edit2 size={13} className="text-slate-400" /></button></td>
                </tr>
              );
            })}
          </tbody>
      ...
      <Modal isOpen={showForm} onClose={() => { setShowForm(false); setEditItem(null); }} title={editItem ? "Editar Contrato" : "Novo Contrato"} size="lg">
        ...
        {selected && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[["CNPJ", selected.cnpj], ["Endereço Instalação", selected.endereco_instalacao], ["Produto", getProductLabel(selected.produto)], ["Valor Mensal", `R$ ${(parseFloat(selected.valor_mensal) || 0).toLocaleString("pt-BR")}`], ["Data Assinatura", selected.data_assinatura ? new Date(selected.data_assinatura + "T12:00:00").toLocaleDateString("pt-BR") : "—"], ["Data Ativação", selected.data_ativacao ? new Date(selected.data_ativacao + "T12:00:00").toLocaleDateString("pt-BR") : "—"], ["Prazo", `${selected.prazo_meses} meses`], ["Data Fim", selected.dataFim ? selected.dataFim.toLocaleDateString("pt-BR") : "—"], ["Multa Rescisão", selected.multa_rescisao]].map(([l, v]) => <div key={l}><span className="text-[10px] text-slate-500 uppercase">{l}</span><p className="text-sm text-white">{v || "—"}</p></div>)}
              <div><span className="text-[10px] text-slate-500 uppercase">Proteção</span><div className="mt-0.5"><Badge color={selected.renovacao_automatica ? "#10b981" : "#ef4444"}>{selected.renovacao_automatica ? "Protegido — Renovação Auto" : "Desprotegido"}</Badge></div></div>
            </div>
            <div className="border-t border-slate-700 pt-3">
              <h4 className="text-xs font-semibold text-white mb-2">Responsáveis</h4>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
...
const ContratoForm = ({ initial, onSave, onCancel }) => {
  const [form, setForm] = useState(initial || { empresa: "", cnpj: "", endereco_instalacao: "", produto: "fibra", valor_mensal: "", data_assinatura: "", data_ativacao: "", prazo_meses: "", status: "ativo", protegido: true, renovacao_automatica: false, multa_rescisao: "30%", responsavel_tecnico_nome: "", responsavel_tecnico_email: "", responsavel_tecnico_telefone: "", responsavel_financeiro_nome: "", responsavel_financeiro_email: "", responsavel_financeiro_telefone: "", responsavel_comercial_nome: "", responsavel_comercial_email: "", responsavel_comercial_telefone: "" });
  const set = (k, v) => setForm(p => ({ ...p, [k]: v }));
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Select label="Multa Rescisão" value={form.multa_rescisao} onChange={v => set("multa_rescisao", v)} options={[{ value: "30%", label: "30%" }, { value: "50%", label: "50%" }]} />
      </div>
      <div className="flex gap-4 items-center">
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" checked={form.renovacao_automatica} onChange={e => set("renovacao_automatica", e.target.checked)} className="rounded bg-slate-800 border-slate-600 text-amber-500 focus:ring-amber-500" />
          <span className="text-xs text-slate-300">Renovação Automática</span>
        </label>
        <Badge color={form.renovacao_automatica ? "#10b981" : "#ef4444"}>{form.renovacao_automatica ? "Protegido" : "Desprotegido"}</Badge>
      </div>
...
const ViabilidadesPage = ({ data, setData }) => {
  ...
  const save = (form) => {
    if (editItem) { setData(prev => ({ ...prev, viabilidades: prev.viabilidades.map(v => v.id === editItem.id ? { ...v, ...form } : v) })); }
    else { setData(prev => ({ ...prev, viabilidades: [...prev.viabilidades, { ...form, id: uid(), status_analise: "pendente" }] })); }
    setShowForm(false); setEditItem(null);
  };

  const approve = (id, field, userId) => {
    setData(prev => ({ ...prev, viabilidades: prev.viabilidades.map(v => {
      if (v.id !== id) return v;
  ...
  const reject = (id) => { setData(prev => ({ ...prev, viabilidades: prev.viabilidades.map(v => v.id === id ? { ...v, status_analise: "rejeitado" } : v) })); };
  ...
  return (
    <div className="space-y-4">
        ...
        {viabilidades.map(v => {
          ...
          return (
            <Card key={v.id} onClick={() => setSelected(v)} className="hover:border-amber-500/30">
              <div className="flex items-start justify-between mb-2">
                <div><p className="text-sm font-semibold text-white">{v.endereco}</p><p className="text-[10px] text-slate-500">{cliente?.nome || "—"}</p></div>
              ...
              {v.custo_instalacao && <p className="text-xs text-amber-400 mt-1">Custo: R$ {parseFloat(v.custo_instalacao).toLocaleString("pt-BR")}</p>}
            </Card>
          );
        })}
      ...
      <Modal isOpen={showForm} onClose={() => { setShowForm(false); setEditItem(null); }} title={editItem ? "Editar Viabilidade" : "Nova Viabilidade"} size="lg">
      ...
      <Modal isOpen={!!selected} onClose={() => setSelected(null)} title="Análise de Viabilidade" size="lg">
        {selected && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[["Endereço", selected.endereco], ["Tipo Conexão", getProductLabel(selected.tipo_conexao)], ["Velocidade", selected.velocidade], ["Prazo", selected.prazo], ["Operadora Atual", selected.operadora_atual], ["Metragem", selected.metragem], ["Equipamentos", selected.equipamentos], ["Custo Instalação", selected.custo_instalacao ? `R$ ${parseFloat(selected.custo_instalacao).toLocaleString("pt-BR")}` : "—"]].map(([l, v]) => <div key={l}><span className="text-[10px] text-slate-500 uppercase">{l}</span><p className="text-sm text-white">{v || "—"}</p></div>)}
              <div><span className="text-[10px] text-slate-500 uppercase">Motivo</span><div className="mt-0.5">{selected.motivo ? <Badge color={MOTIVOS_VIABILIDADE.find(m => m.value === selected.motivo)?.color || "#6b7280"}>{MOTIVOS_VIABILIDADE.find(m => m.value === selected.motivo)?.label || selected.motivo}</Badge> : <p className="text-sm text-white">—</p>}</div></div>
              <div><span className="text-[10px] text-slate-500 uppercase">Rede</span><div className="mt-0.5">{selected.rede ? <Badge color={selected.rede === "on_net" ? "#10b981" : "#8b5cf6"}>{selected.rede === "on_net" ? "ON-NET" : "OFF-NET"}</Badge> : <p className="text-sm text-white">—</p>}</div></div>
            </div>
              ...
              <div className="flex gap-3">
                <div className="flex-1 bg-slate-800/30 rounded-lg p-3">
                  ...
                  {selected.status_analise === "pendente" && (
                    <div className="flex gap-2 mt-2">
                      <Button size="sm" variant="success" onClick={() => { approve(selected.id, "analise", SEED_USERS[4].id); setSelected({ ...selected, status_analise: "aprovado" }); }}>Aprovar</Button>
                ...
                <div className="flex-1 bg-slate-800/30 rounded-lg p-3">
                  ...
                  {!selected.aprovado_por && selected.status_analise === "aprovado" && (
                    <div className="mt-2"><Button size="sm" variant="success" onClick={() => { approve(selected.id, "custo", SEED_USERS[0].id); setSelected({ ...selected, aprovado_por: SEED_USERS[0].id }); }}>Aprovar Custo</Button></div>
                  )}
                </div>
              </div>
            </div>
...
const ViabilidadeForm = ({ initial, data, onSave, onCancel }) => {
  const [form, setForm] = useState(initial || { cliente_id: "", endereco: "", latitude: "", longitude: "", tipo_conexao: "fibra", velocidade: "", prazo: "", operadora_atual: "", equipamentos: "", metragem: "", custo_instalacao: "", motivo: "", rede: "", imagem_url: "" });
  const set = (k, v) => setForm(p => ({ ...p, [k]: v }));
  const handleImageUpload = (e) => { const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onloadend = () => set("imagem_url", reader.result); reader.readAsDataURL(file); } };

  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <Select label="Cliente" value={form.cliente_id} onChange={v => set("cliente_id", v)} options={data.clientes.map(c => ({ value: c.id, label: c.nome }))} />
        <Select label="Tipo Conexão" value={form.tipo_conexao} onChange={v => set("tipo_conexao", v)} options={TIPOS_PRODUTO.map(t => ({ value: t.value, label: t.label }))} />
        <Select label="Motivo" value={form.motivo} onChange={v => set("motivo", v)} options={MOTIVOS_VIABILIDADE.map(m => ({ value: m.value, label: m.label }))} />
        <Select label="Rede" value={form.rede} onChange={v => set("rede", v)} options={[{ value: "on_net", label: "ON-NET" }, { value: "off_net", label: "OFF-NET" }]} />
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-3">
          <label className="cursor-pointer inline-flex items-center gap-1.5 px-3 py-2 bg-slate-800/50 border border-slate-700 rounded-lg text-xs text-slate-300 hover:bg-slate-700 hover:text-white transition-colors">
            <Upload size={14} />{form.imagem_url ? "Trocar imagem" : "Anexar imagem"}
            <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
          </label>
          {form.imagem_url && <div className="flex items-center gap-2"><div className="w-10 h-10 rounded-md overflow-hidden bg-slate-800 border border-slate-600"><img src={form.imagem_url} alt="Preview" className="w-full h-full object-cover" /></div><button onClick={() => set("imagem_url", "")} className="text-[10px] text-red-400 hover:text-red-300">Remover</button></div>}
        </div>
      </div>
      <div className="flex gap-2 justify-end pt-2">
        <Button variant="secondary" onClick={onCancel}>Cancelar</Button>
...
const TarefasPage = ({ data, setData }) => {
  ...
  const filtered = tarefas.filter(t => { if (filterStatus && t.status !== filterStatus) return false; if (filterPrior && t.prioridade !== filterPrior) return false; return true; });

  const save = (form) => {
    if (editItem) { setData(prev => ({ ...prev, tarefas: prev.tarefas.map(t => t.id === editItem.id ? { ...t, ...form } : t) })); }
    else { setData(prev => ({ ...prev, tarefas: [...prev.tarefas, { ...form, id: uid() }] })); }
    setShowForm(false); setEditItem(null);
  };

  const toggleStatus = (id) => { setData(prev => ({ ...prev, tarefas: prev.tarefas.map(t => { if (t.id !== id) return t; const next = { aberta: "em_progresso", em_progresso: "concluida", concluida: "aberta", cancelada: "aberta" }; return { ...t, status: next[t.status] }; }) })); };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 justify-between flex-wrap">
        <div className="flex gap-2 flex-wrap">
          <Select value={filterStatus} onChange={setFilterStatus} options={Object.entries(statusLabels).map(([v, l]) => ({ value: v, label: l }))} placeholder="Todos status" />
          <Select value={filterPrior} onChange={setFilterPrior} options={Object.entries(PRIORIDADES).map(([v, p]) => ({ value: v, label: p.label }))} placeholder="Todas prioridades" />
        </div>
        ...
        {filtered.map(t => {
          ...
          return (
            <div key={t.id} className={`flex items-center gap-3 p-3 rounded-lg border transition-all ${atrasada ? "bg-red-500/5 border-red-500/20" : "bg-slate-800/30 border-slate-700/50 hover:bg-slate-800/50"}`}>
              <button onClick={() => toggleStatus(t.id)} className={`flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${t.status === "concluida" ? "bg-emerald-500 border-emerald-500" : "border-slate-600 hover:border-amber-500"}`}>
              ...
              <button onClick={() => { setEditItem(t); setShowForm(true); }} className="p-1 hover:bg-slate-700 rounded"><Edit2 size={13} className="text-slate-400" /></button>
            </div>
          );
        })}
      ...
      <Modal isOpen={showForm} onClose={() => { setShowForm(false); setEditItem(null); }} title={editItem ? "Editar Tarefa" : "Nova Tarefa"}>
...
const TarefaForm = ({ initial, data, onSave, onCancel }) => {
  const [form, setForm] = useState(initial || { titulo: "", descricao: "", tipo: "ligacao", status: "aberta", prioridade: "media", data_vencimento: "", atribuido_a: "", oportunidade_id: "", cliente_id: "" });
  const set = (k, v) => setForm(p => ({ ...p, [k]: v }));
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Select label="Tipo" value={form.tipo} onChange={v => set("tipo", v)} options={[{ value: "ligacao", label: "Ligação" }, { value: "reuniao", label: "Reunião" }, { value: "email", label: "Email" }, { value: "proposta", label: "Proposta" }, { value: "contrato", label: "Contrato" }]} />
        <Select label="Status" value={form.status} onChange={v => set("status", v)} options={[{ value: "aberta", label: "Aberta" }, { value: "em_progresso", label: "Em Progresso" }, { value: "concluida", label: "Concluída" }, { value: "cancelada", label: "Cancelada" }]} />
        <Select label="Prioridade" value={form.prioridade} onChange={v => set("prioridade", v)} options={Object.entries(PRIORIDADES).map(([v, p]) => ({ value: v, label: p.label }))} />
        ...
        <Select label="Atribuir a" value={form.atribuido_a} onChange={v => set("atribuido_a", v)} options={SEED_USERS.map(u => ({ value: u.id, label: u.nome }))} />
        <Select label="Cliente" value={form.cliente_id} onChange={v => set("cliente_id", v)} options={data.clientes.map(c => ({ value: c.id, label: c.nome }))} />
      </div>
      <div className="flex gap-2 justify-end pt-2">
        <Button variant="secondary" onClick={onCancel}>Cancelar</Button>
...
const ProdutosPage = ({ data, setData }) => {
  ...
  const save = (form) => {
    if (editItem) { setData(prev => ({ ...prev, produtos: prev.produtos.map(p => p.id === editItem.id ? { ...p, ...form } : p) })); }
    else { setData(prev => ({ ...prev, produtos: [...prev.produtos, { ...form, id: uid(), ativo: true }] })); }
    setShowForm(false); setEditItem(null);
  };
  const toggle = (id) => { setData(prev => ({ ...prev, produtos: prev.produtos.map(p => p.id === id ? { ...p, ativo: !p.ativo } : p) })); };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {produtos.map(p => (
          <Card key={p.id} className={`${!p.ativo ? "opacity-50" : ""}`}>
            <div className="flex items-start justify-between mb-2">
              <div><p className="text-sm font-semibold text-white">{p.nome}</p><p className="text-[10px] text-slate-500 mt-0.5">{p.descricao}</p></div>
              <Badge color={getProductColor(p.tipo)}>{getProductLabel(p.tipo)}</Badge>
            </div>
            <p className="text-lg font-bold text-amber-400 mb-3">R$ {(parseFloat(p.preco_base) || 0).toLocaleString("pt-BR")}<span className="text-xs font-normal text-slate-500">/mês</span></p>
            <div className="flex gap-2">
              <Button size="sm" variant="secondary" icon={Edit2} onClick={() => { setEditItem(p); setShowForm(true); }}>Editar</Button>
              <Button size="sm" variant={p.ativo ? "ghost" : "success"} onClick={() => toggle(p.id)}>{p.ativo ? "Desativar" : "Ativar"}</Button>
            </div>
          </Card>
        ))}
      </div>
      <Modal isOpen={showForm} onClose={() => { setShowForm(false); setEditItem(null); }} title={editItem ? "Editar Produto" : "Novo Produto"}>
...
const ProdutoForm = ({ initial, onSave, onCancel }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <Textarea label="Descrição" value={form.descricao} onChange={v => set("descricao", v)} rows={2} />
      <div className="grid grid-cols-2 gap-3">
        <Select label="Tipo" value={form.tipo} onChange={v => set("tipo", v)} options={TIPOS_PRODUTO.map(t => ({ value: t.value, label: t.label }))} />
...
const RelatoriosPage = ({ data }) => {
  ...
  const vendasData = ETAPAS.map(e => ({ etapa: e.label, qtd: oportunidades.filter(o => o.etapa === e.id).length, valor: oportunidades.filter(o => o.etapa === e.id).reduce((s, o) => s + (parseFloat(o.valor_mensal) || 0), 0) }));
  const contratosResumo = { total: contratos.length, ativos: contratos.filter(c => c.status === "ativo").length, mrr: contratos.reduce((s, c) => s + (parseFloat(c.valor_mensal) || 0), 0), arr: contratos.reduce((s, c) => s + (parseFloat(c.valor_mensal) || 0), 0) * 12 };
  const contratosPorProduto = TIPOS_PRODUTO.map(t => ({ name: t.label, value: contratos.filter(c => c.produto === t.value).length, color: t.color })).filter(d => d.value > 0);
  const viabResumo = { total: viabilidades.length, aprovadas: viabilidades.filter(v => v.status_analise === "aprovado").length, rejeitadas: viabilidades.filter(v => v.status_analise === "rejeitado").length, pendentes: viabilidades.filter(v => v.status_analise === "pendente").length };

  return (
    <div className="space-y-4">
      <Tab tabs={[{ id: "vendas", label: "Vendas" }, { id: "contratos", label: "Contratos" }, { id: "receita", label: "Receita" }, { id: "viabilidades", label: "Viabilidades" }]} active={tab} onChange={setTab} />
      ...
      {tab === "contratos" && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            {contratosPorProduto.slice(0, 4).map(cp => <KPICard key={cp.name} title={cp.name} value={cp.value} icon={Wifi} color={cp.color} />)}
          </div>
          <Card>
            <h3 className="text-sm font-semibold text-white mb-3">Contratos por Produto</h3>
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={contratosPorProduto.length > 0 ? contratosPorProduto : [{ name: "Sem dados", value: 1, color: "#334155" }]} cx="50%" cy="50%" innerRadius={40} outerRadius={70} paddingAngle={5} dataKey="value">
                  {(contratosPorProduto.length > 0 ? contratosPorProduto : [{ color: "#334155" }]).map((entry, i) => <Cell key={i} fill={entry.color} />)}
                </Pie>
                <Tooltip contentStyle={{ background: "#1e293b", border: "1px solid #334155", borderRadius: "8px", fontSize: "12px", color: "#fff" }} />
                <Legend iconType="circle" wrapperStyle={{ fontSize: "12px" }} />
              </PieChart>
      ...
      {tab === "receita" && (
        <div className="space-y-4">
          ...
          <Card>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={Array.from({ length: 12 }, (_, i) => ({ mes: new Date(2026, i).toLocaleDateString("pt-BR", { month: "short" }), receita: contratosResumo.mrr * (1 + i * 0.02) }))}>
                <CartesianGrid strokeDasharray="3 3" stroke="#334155" />
                <XAxis dataKey="mes" stroke="#64748b" fontSize={11} />
                <YAxis stroke="#64748b" fontSize={11} tickFormatter={v => `${(v / 1000).toFixed(0)}k`} />
                <Tooltip contentStyle={{ background: "#1e293b", border: "1px solid #334155", borderRadius: "8px", fontSize: "12px", color: "#fff" }} formatter={v => [`R$ ${v.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}`, "Receita"]} />
      ...
      {tab === "viabilidades" && (
        <div className="space-y-4">
          ...
          <Card>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={[{ name: "Aprovadas", value: viabResumo.aprovadas || 0 }, { name: "Rejeitadas", value: viabResumo.rejeitadas || 0 }, { name: "Pendentes", value: viabResumo.pendentes || 0 }].filter(d => d.value > 0)} cx="50%" cy="50%" innerRadius={40} outerRadius={70} paddingAngle={5} dataKey="value">
...
// ==================== MAIN APP ====================
const pageTitles = { dashboard: "Dashboard", pipeline: "Pipeline de Vendas", clientes: "Clientes", contratos: "Contratos", viabilidades: "Viabilidade Técnica", tarefas: "Tarefas", produtos: "Produtos", relatorios: "Relatórios" };
...
export default function App() {
  ...
  const [data, setData] = useState({ clientes: SEED_CLIENTES, oportunidades: SEED_OPORTUNIDADES, contratos: SEED_CONTRATOS, produtos: SEED_PRODUTOS, tarefas: SEED_TAREFAS, atividades: SEED_ATIVIDADES, viabilidades: SEED_VIABILIDADES });
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      ...
      try {
        ...
        const [, clientes, oportunidades, contratos, produtos, tarefas, atividades, viabilidades] = results;
        setData(prev => ({
          clientes: Array.isArray(clientes) && clientes.length > 0 ? clientes : prev.clientes,
          oportunidades: Array.isArray(oportunidades) ? oportunidades : prev.oportunidades,
          contratos: Array.isArray(contratos) ? contratos : prev.contratos,
          produtos: Array.isArray(produtos) && produtos.length > 0 ? produtos : prev.produtos,
          tarefas: Array.isArray(tarefas) ? tarefas : prev.tarefas,
          atividades: Array.isArray(atividades) ? atividades : prev.atividades,
          viabilidades: Array.isArray(viabilidades) ? viabilidades : prev.viabilidades,
        }));
        setDbStatus("online");
      } catch (e) { console.warn("[Supabase] Load failed:", e); if (!cancelled) setDbStatus("offline"); }
    }
    loadAll();
    ...
    setData(prev => {
      ...
      if (dbStatus === "online") {
        ...
        tableNames.forEach(table => {
          const oldArr = prev[table]; const newArr = next[table];
          if (oldArr === newArr) return;
          const oldIds = new Set(oldArr.map(r => r.id));
          newArr.filter(r => !oldIds.has(r.id)).forEach(item => {
            const clean = { ...item }; delete clean.dataFim; delete clean.diasRestantes; delete clean.statusCalc;
            sb.insert(table, clean).then(res => { if (res) { setSyncing(true); setTimeout(() => setSyncing(false), 800); } });
          });
          const newIds = new Set(newArr.map(r => r.id));
          oldArr.filter(r => !newIds.has(r.id)).forEach(item => { sb.remove(table, item.id); });
          newArr.forEach(newItem => {
            const oldItem = oldArr.find(o => o.id === newItem.id);
            if (!oldItem) return;
            if (JSON.stringify(oldItem) !== JSON.stringify(newItem)) {
              const clean = { ...newItem }; delete clean.dataFim; delete clean.diasRestantes; delete clean.statusCalc;
              sb.update(table, newItem.id, clean).then(res => { if (res) { setSyncing(true); setTimeout(() => setSyncing(false), 800); } });
            }
          });
        });
      }
